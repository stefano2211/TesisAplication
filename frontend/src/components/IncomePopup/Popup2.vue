      ...
      <div class="modal-body">
        <form @submit="onSubmit">
          ...
          <div class="mb-3">
            ...
            <select name="select" type="text" class="form-control" id="recipient-name" v-model.trim="form.typeIncome">
              <option v-for="Income in Income" :key="Income.id">{{Income.name}}</option>
            </select>
          </div>
          <div class="mb-3">
...
export default {
	...
	methods: {
		onSubmit (e) {
			...
			const path = 'http://127.0.0.1:8000/Income/'
			axios.post(path, this.form).then((response) => {
				this.form.name = response.data.name
				this.form.description = response.data.description
				this.form.price = response.data.price
				this.form.typeBank = response.data.typeBank
				this.form.typeIncome = response.data.typeIncome
	...
	mounted () {
		...
		axios.get('http://127.0.0.1:8000/CodeIncome/')
			.then(response => {
